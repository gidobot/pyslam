...
from utils_knfu_slam import normalizeFisheye, normalizePerspective, scaleImprovedSIFT
...
uw_root_dir = "/media/Data/PSTAR/Reconstruction_Datasets/UWHandles/20181221/SlopeAndMagneticSeep/set2/tagslam"
# uw_root_dir = "/media/Data/PSTAR/Reconstruction_Datasets/UWHandles/20181220/Mound12/set1/arrangement3/tagslam"
def read_pose(idx):
...
if test_type == 'underwater':
    ...
    img1 = cv2.imread(osp.join(uw_root_dir, "images/masked", str(idx) + '_fish.png')) # queryImage
    img2 = cv2.imread(osp.join(uw_root_dir, "images/masked", str(idx) + '_left.png')) # trainImage
    img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2RGB)
    img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2RGB)
...
# scaleImprovedSIFT(img1, img2, feature_tracker)
...
if kps1_matched.shape[0] > 10:
    ...
    if model_fitting_type == 'homography': 
        ...
        # pose_ref = refiner.refineRobustOnMask(kps2_matched_normalized, kps1_matched_normalized, mask.flatten(), pose)
